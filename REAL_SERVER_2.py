...
class ChatServer:
    ...
    def handle_client(self, client_socket, client_address, username, room):
        ...
        client_socket.settimeout(300)  # Timeout after 300 seconds of inactivity
        while True:
            try:
                message = client_socket.recv(1024).decode('utf-8')
                if not message:
                ...
                if message.lower() == "leave":
                    print(Fore.YELLOW + f"[STATUS] {username} IP {client_address} has been left the room {room} at {timestamp}" + Style.RESET_ALL)
                    self.send_to_room(Fore.YELLOW + f"{username} has been left the room at {timestamp}" + Style.RESET_ALL, room)
                    self.send_to_room("", room)
                    self.remove_client(client_socket, room)
                    self.handle_client(client_socket, client_address, username, room)
                    return

                if message.lower() == "exit":
                    print(Fore.RED + f"[STATUS] {username} IP {client_address} has been left the program at {timestamp}" + Style.RESET_ALL)
                    self.send_to_room(Fore.RED + f"{username} has been left the program at {timestamp}" + Style.RESET_ALL, room)
                    self.remove_client(client_socket, room)
                    self.send_to_client(Fore.RED + "You have exited the chat." + Style.RESET_ALL, client_socket)
                    client_socket.close()
                    return
                ...
                print(Fore.RED + f"[STATUS] {username} IP {client_address} has been disconnected due to inactivity." + Style.RESET_ALL)
                self.send_to_room(Fore.RED + f"{username} has been disconnected due to inactivity." + Style.RESET_ALL, room)
                self.remove_client(client_socket, room)
                break
            except:
                break
